...
class topic_test():
    ...
    def create_producer(self):
        ...
        try:
            ...
            b=self.channel.exchange_declare(exchange="direct_type", exchange_type="direct", durable=True, auto_delete=False)
            c=self.channel.queue_bind(queue="direct_queue", exchange="direct_type",routing_key="jason")  
            print("创建queue成功",a)
    ...
    #                               body='测试数据_'+str(i)) # 消息内容
                self.channel.basic_publish(exchange='direct_type',
                                  routing_key='jason',
                                  body='测试数据_'+str(i),
